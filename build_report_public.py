...
@page {
    ...
    @bottom-center {
        content: "SAENAL Market — E-Commerce Analytics Case Study";
        font-size: 8pt;
        color: #888;
...
<div class="cover">
  <div class="tag">INTERNSHIP PROJECT · UPDATED ANALYSIS · PUBLIC VERSION</div>
  <h1>SAENAL Market</h1>
  <h2>E-Commerce Analytics Case Study: RFM Segmentation, Vendor Risk &amp; Demand Forecasting</h2>
  <div class="meta">
  ...
  <div class="footnote">This is a public version prepared for portfolio use. Vendor names have been replaced
  with generic labels (Vendor A, B, C...) and absolute revenue figures have been converted to an index
  (2021 total = 100) or relative percentages to protect business-sensitive information. All relative
  findings, ratios, and model results are unchanged from the internal version.</div>
</div>
...
By listing count the platform looks fine (HHI 128). By revenue, the top vendor alone (Vendor A) holds 21.8%
...
<tr><td><b>Peak Year</b></td><td>2022 revenue (sales-ledger basis, indexed to 2021 = 100) hit an index of
~480, the highest in the dataset. 2023 dropped 10.6% to an index of ~428. 2024's partial year (Jan–Jul) is
at an index of ~303, which annualizes to roughly 493–521.</td></tr>
...
has been climbing since 2022 (633 → 1,928 by 2024). The top vendor, Vendor A, holds 21.8% of platform revenue
...
<p>The bracket-extraction regex pulled the entire promotional campaign text out of a milestone SKU name (a
one-day sales-milestone event name, formatted the same way as a vendor tag) and treated it as a separate
vendor. That one mislabeled bucket carried revenue equal to roughly three-quarters of 건강식품's entire
2023 category revenue, enough to change who the #1 vendor is by revenue. It's folded back into the real
vendor, Vendor A, before the concentration analysis in Section 9.</p>
...
match the product master list on <code>Category</code>, so a large chunk of its revenue had landed in the
catch-all <code>Unknown</code> bucket instead of its real category. Once the vendor label was corrected, the
category-fill logic assigned it correctly to 건강식품, since that's what the rest of Vendor A's catalog is
categorized as.</p>
""" +
callout("Before / after", "The 'Unknown' category's 2023 revenue shrinks by about 97% once this is fixed. "
    "건강식품's 2023 growth reading flips from −73.8% to +4.7% year-over-year. That's the difference "
    "between a category that looks like it's dying and one that's actually stable. See Section 6.2 for the "
    "corrected matrix.", "fix") +
"""
...
""" +
fig("chart_revenue_trends_idx.png", "Figure 1. Monthly sales trends by year (left) and annual totals (right), "
    "2021–2024.") +
"""
<table>
<tr><th>Year</th><th>Revenue Index (2021 = 100)</th><th>Coverage</th><th>Annualized Estimate</th></tr>
<tr><td>2021</td><td>100</td><td>Jul–Dec (6 mo)</td><td>~200</td></tr>
<tr><td>2022</td><td>479</td><td>Full Year</td><td>479 (actual peak)</td></tr>
<tr><td>2023</td><td>428</td><td>Full Year</td><td>428 (−10.6% YoY)</td></tr>
<tr><td>2024</td><td>303</td><td>Jan–Jul (7 mo, partial)</td><td>~493–521 annualized</td></tr>
</table>
...
""" +
fig("chart_avg_monthly_idx.png", "Figure 2. Average monthly revenue across all years. December, June, and "
    "November are consistent peaks; July is the structural trough.") +
"""
<p>December, June, and November are consistent peaks across multiple years, which points to structural
demand cycles rather than one-off promotions. July and August sit about 50% below peak months, a
predictable, calendar-certain gap that's still underused as a planning input (see Recommendations).</p>
""")
...
""" +
fig("chart_category_revenue_idx.png", "Figure 3. Revenue by year and category. 건강 and 건강식품's rise "
    "dominates the mix; about 30 other categories compete for the remainder.") +
fig("chart_category_top10_idx.png", "Figure 4. Category frequency (product count, left) and top-10 products by "
    "total revenue (right).") +
"""
...
"""
<table>
<tr><th>Category</th><th>YoY Growth</th><th>Share of Category Revenue</th><th>Quadrant</th></tr>
<tr><td>건강</td><td>+177.0%</td><td>31.0%</td><td>Star</td></tr>
<tr><td>건강식품</td><td>+4.7%</td><td>30.7%</td><td>Star</td></tr>
<tr><td>국/탕/찌개/면</td><td>−44.2%</td><td>6.8%</td><td>Cash Cow</td></tr>
<tr><td>뷰티/다이어트</td><td>+58.8%</td><td>5.8%</td><td>Star</td></tr>
<tr><td>수산/건어물/김</td><td>−27.1%</td><td>4.0%</td><td>Cash Cow</td></tr>
<tr><td>밥/반찬/요리</td><td>−53.2%</td><td>3.8%</td><td>Cash Cow</td></tr>
<tr><td>식품</td><td>+74.8%</td><td>1.9%</td><td>Question Mark</td></tr>
</table>
...
""" +
fig("chart_discount_revenue_idx.png", "Figure 7. Total revenue by discount/membership segment. 회원 (Member) "
    "dominates.") +
"""
<table>
<tr><th>Segment</th><th>Share of Revenue</th><th>Interpretation</th></tr>
<tr><td>회원 (Member)</td><td>~66%</td><td>The clear revenue engine: higher frequency, larger basket
size, lower price sensitivity.</td></tr>
<tr><td>할인불가 (Non-Member)</td><td>~24%</td><td>Distant second. The conversion opportunity.</td></tr>
<tr><td>Unknown</td><td>~9%</td><td>Lost segmentation intelligence. Membership should be captured at
checkout.</td></tr>
</table>
...
be worth an estimated 2–4% lift to total platform revenue.</p>
""")
...
""" +
fig("chart_pairplot_idx.png", "Figure 9. Pairplot across key variables, top-5 categories.") +
"""
""")

HTML_PARTS.append("""
<h1 class="section">9. Vendor Concentration — Quantified</h1>
<p>The original report flagged vendor risk using a listing-frequency chart: Vendor B had far more rows than
...
""" +
fig("chart_vendor_concentration_public.png", "Figure 10. HHI across four concentration bases (left) and the top "
...
<tr><td>Listing frequency (v2 metric)</td><td>Vendor B</td><td>8.3%</td><td>128</td><td>16.1%</td>
<td>Looks competitive, but measures tenure, not weight.</td></tr>
<tr><td>Unique SKU count</td><td>Vendor C</td><td>3.2%</td><td>56</td><td>10.8%</td><td>Even more diffuse.
No single vendor dominates by SKU count.</td></tr>
<tr><td>Order volume</td><td>Vendor D</td><td>15.1%</td><td>368</td><td>30.3%</td><td>More concentrated
than SKU count would suggest.</td></tr>
<tr><td><b>Revenue (economic weight)</b></td><td><b>Vendor A</b></td><td><b>21.8%</b></td><td><b>997</b></td>
<td><b>53.8%</b></td><td><b>The metric that actually matters for supply risk.</b></td></tr>
</table>
<p>Vendor A is the platform's largest vendor by revenue at 21.8% of the total, above a 20% single-vendor
exposure guideline, and it holds 78.1% of the entire 건강식품 category's revenue. If Vendor A exits,
...
""" +
fig("chart_decomposition_idx.png", "Figure 11. Seasonal decomposition of monthly revenue: trend plus yearly "
    "seasonal component.") +
"""
...
data point.</p>
""" +
fig("chart_model_comparison_idx.png", "Figure 12. Held-out forecast accuracy: SARIMA vs. Prophet, last 6 "
    "months.") +
"""
<table>
<tr><th>Model</th><th>MAE (index)</th><th>RMSE (index)</th><th>MAPE</th></tr>
<tr><td><b>SARIMA(1,1,1)(1,1,1,12)</b></td><td>8.5</td><td>13.4</td><td><b>23.9%</b></td></tr>
<tr><td>Prophet</td><td>19.8</td><td>23.7</td><td>41.0%</td></tr>
</table>
<p class="small">MAE/RMSE are shown on the same revenue-index scale as the charts (2021 total = 100), not
in currency, so only the relative gap between models matters here.</p>
...
""" +
fig("chart_sarima_fit_idx.png", "Figure 13. SARIMA fit validation: observed vs. fitted, full history.") +
fig("chart_forecast_idx.png", "Figure 14. 12-month forward forecast with 95% confidence interval, refit on the "
    "complete history.") +
"""
<p>Monthly peaks in late 2025 are projected roughly 25–40% above 2024's observed peaks, driven by trend
accumulation plus seasonal uplift. The confidence interval widens noticeably past Q3 2025, so quarterly
recalibration is still the right approach for planning purposes.</p>
""")
...
<tr><td>Revenue Trajectory</td><td>2022 peak (index 479 vs. 2021 = 100), 2023 contraction (−10.6%), 2024
recovery pace of roughly index 493–521 annualized.</td></tr>
...
conversion rate would be worth an estimated 2–4% lift to total platform revenue.</td></tr>
...
<h3 class="subsub">⑤ Cap Single-Vendor Exposure, Starting with Vendor A — High Priority</h3>
<p>Vendor A holds 78% of 건강식품 category revenue and 21.8% of platform revenue, both above a recommended
...
of this report shows a single bracket-extraction bug moved a chunk of revenue equal to about three-quarters
of one category's annual total into the wrong category.
...
new findings from this update, the Declining Stars SKU segment and the Vendor A vendor exposure, are the most
...
with open('SAENAL_Market_Ecommerce_Analytics_CaseStudy.html', 'w', encoding='utf-8') as f:
    f.write(FULL_HTML)

weasyprint.HTML(string=FULL_HTML, base_url='.').write_pdf('SAENAL_Market_Ecommerce_Analytics_CaseStudy.pdf')
print('Wrote SAENAL_Market_Ecommerce_Analytics_CaseStudy.pdf,', len(FULL_HTML), 'chars of HTML')
